...
pushd $OLIMEX_EXTENSIONS_DIR > /dev/null
  ...
  if [ -f build/micro-ROS.bin ]; then
    ...
    openocd -f interface/ftdi/olimex-arm-usb-tiny-h.cfg -f target/stm32f4x.cfg -c init -c "reset halt" -c "flash write_image erase build/micro-ROS.bin 0x08000000" -c "reset"
  else
